...
async function getPrivacyPolicy(locale: string) {
  try {
    const res = await fetch(`${STRAPI_API_URL}/privacy-policy?locale=${locale}`, { cache: 'no-store' });
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  if (!data) {
    return { title: "Privacy Policy | Dengarkan.id" };
  }

  const attrs = data.attributes || data;
  return {
    title: `${attrs.title || "Privacy Policy"} | Dengarkan.id`,
  };
}
      ...
      <section className="legal-doc-section">
        <div className="container" style={{ maxWidth: '800px', margin: '0 auto', padding: '0 24px' }}>
          <div className="legal-doc-header">
            <h1 className="legal-doc-title">{attrs.title || "Privacy Policy"}</h1>
