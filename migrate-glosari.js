...
async function run() {
    console.log(`Starting migration of ${data.length} records...`);
    let count = 0;
    for (const item of data) {
        ...
        try {
            const res = await fetch('http://localhost:1337/api/glosariums', {
                method: 'POST',
            ...
            if (res.ok) {
                ...
                console.error(`Failed to insert ${item.t}:`, await res.text());
            }
        } catch (err) {
            console.error(`Error connecting for ${item.t}:`, err.message);
        }
    }
    console.log(`Migration complete! Successfully inserted ${count}/${data.length} records.`);
}
